...
public class LandmarkServiceImpl extends RemoteServiceServlet implements LandmarkService 
{
...
//		List<Landmarks> landmarklist = (List<Landmarks>) pm.newQuery(Landmarks.class, "this.placename =="+ "'" + text +"'").execute();
	...
	public String loadStationGeneric(HashMap hmap) {
		
		try{
			Landmark landmark=  null/*new Landmark(hmap)*/;
			pm.makePersistent(landmark);
		   } finally
		   	{
			   //pm.close();
